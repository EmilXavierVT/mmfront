  ...
  return (
    <header className="topbar">
      <div className="nav-left">
        <a className="icon-btn" href="/" onClick={(event) => navigate(event, 'home')} aria-label="Home"><Icon name="home" /></a>
        <a className={`nav-link ${active==='catering'?'active':''}`} href="/catering" onClick={(event) => navigate(event, 'catering')}>
          <Icon name="fork" size={16}/><span>Catering</span>
        </a>
        <a className={`nav-link ${active==='cleaning'?'active':''}`} href="/cleaning" onClick={(event) => navigate(event, 'cleaning')}>
          <Icon name="spray" size={16}/><span>Cleaning</span>
        </a>
        {isAdmin && (
          <a className={`nav-link ${active==='admin'?'active':''}`} href="/admin" onClick={(event) => navigate(event, 'admin')}>
      ...
      <div className="nav-right">
        {user ? (
          <a className={`account-pill ${active === 'profile' ? 'active' : ''}`} href="/profile" onClick={(event) => navigate(event, 'profile')} aria-label="Profile">
